...
begin
  ...
  if string.IsNullOrEmpty(lbleditStudID.Text) then
    ...
    ShowMessage('Въвели сте несъщестуващ факултетен номер!')
  else if cboxDishes.ItemIndex = -1 then
    ShowMessage('Моля изберете ястие от менюто!')
  else begin
    ...
    ShowMessage('Поръчката е добавена успешно!');
...
begin
  if MessageDlg('Наистина ли искате да се изтрие записът?',
    ...
    ShowMessage('Поръчка номер ' + poruchkaNomer + ' е изтрита!');
